...
void EngineMain::InitModules()
{
	...
	// Shader
	{
		...
		UINT compileFlags = 0;
#endif
		std::wstring mcsFileName = L"C:/Users/jjh/Desktop/Project/JEngine/DX12_MultiThreadFrameBuffer_CBVSRVDescriptor_EachProcessing/shaders_0.hlsl";
		HRESULT hr = S_FALSE;
	...
	// 버텍스 버퍼 생성
	{
		...
		mObj.push_back(new JMesh("square", mpDevice.Get(), nullptr, XMFLOAT3(-0.1f, 0.3f, 0.0f)));
		mObj.push_back(new JMesh("square", mpDevice.Get(), nullptr, XMFLOAT3(-0.7f, -0.2f, 0.0f)));
		mObj.push_back(new JMesh("square", mpDevice.Get(), nullptr, XMFLOAT3(-1.3f, -0.7f, 0.0f)));
	}
	...
	// CreateConstantBufferView
	{
		...
		for (int i = 0; i < mObj.size(); ++i)
		{
			...
			D3D12_GPU_DESCRIPTOR_HANDLE gpuhandle = mpCBVHeap->GetGPUDescriptorHandleForHeapStart();
			gpuhandle.ptr += static_cast<size_t>(i * HandleIncrementSize);
	...
	// CreateTexture
	{
		...
		D3D12_GPU_DESCRIPTOR_HANDLE hGpuDescriptor(mpCBVHeap->GetGPUDescriptorHandleForHeapStart());
		hGpuDescriptor.ptr += static_cast<size_t>(mObj.size() * HandleIncrementSize);
...
void EngineMain::AddBuffe()
{
	...
	D3D12_GPU_DESCRIPTOR_HANDLE gpuhandle = mpCBVHeap->GetGPUDescriptorHandleForHeapStart();
	gpuhandle.ptr += static_cast<size_t>((iLastIndex + 1) * HandleIncrementSize);
...
{
	mTimerClass.Frame();
	mCamera->Update(mTimerClass.GetTime());
	...
	concurrency::parallel_for<UINT>(0, iNumberOfProcessors, [&](UINT threadIndex) 
	{
		...
		{// 리소스 설정, 파이프라인상태 설정
			...
			*/
			...
			ID3D12DescriptorHeap* ppHeaps[] = { mpCBVHeap.Get(), mpSampleHeap.Get() };
