...
void EngineMain::InitModules()
{
	...
	// Shader
	{
		...
		UINT compileFlags = 0;
#endif
		std::wstring mcsFileName = L"C:/Users/jjh/Desktop/Project/JEngine/DX12_AddConstantBufferViewImage/shaders_0.hlsl";
		HRESULT hr = S_FALSE;
	...
	// CreateRootSignature
	{
		...
		// 디스크립트힙에서 디스크립트 데이블러 설정할거면 여기서 샘플러 넣으면 안된다
	...
	{// CreateSampler
		...
		//sampler.MinLOD = -1.0f;
		//sampler.MaxLOD = 1.0f;
		//D3D12_CPU_DESCRIPTOR_HANDLE mSampler(mpCBVHeap->GetCPUDescriptorHandleForHeapStart());
		//UINT HandleIncrementSize = mpDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		//mSampler.ptr += static_cast<size_t>((mObj.size()+1) * HandleIncrementSize);
		////hDescriptor.Offset()
	...
	{// 리소스 설정, 파이프라인상태 설정
		...
		*/
		...
		for (int i = 0; i < mObj.size(); i++)
		{
			...
			SRV.ptr += 2 * mCbvSrvDescriptorSize;// CBV.Offset(i * mCbvSrvDescriptorSize);
